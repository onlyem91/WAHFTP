package nl.hackergames.pizza.client;
...
public class WelcomeRestService {
     ...
     * <p/>
     * By default the URL will point to 'http://localhost:8080/codeApi-server' which should hit your
     ...
     *                  'http://localhost:8080/codeApi-server'
     */
     ...
     */
    public ResponseMessage sayHello(String name) {
        log.info("Saying hello to server using name: {}", name);
        Map<String, Object> values = new HashMap<String, Object>();
        values.put("name", name);
        ResponseMessage responseMessage = restTemplate.getForObject(
                serverUrl + "/welcome?name={name}", ResponseMessage.class, values);
        log.info("Server responded with: {}", responseMessage.getMessage());
